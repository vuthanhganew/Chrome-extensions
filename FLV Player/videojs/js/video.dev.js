...
vjs.Component.prototype.dimension = function(widthOrHeight, num, skipListeners){
  if (num !== undefined) {
    ...
    if(!this.el_)this.el_ = document.getElementById(this.id_);
    // Check if using css width/height (% or px) and adjust

    if ((''+num).indexOf('%') !== -1 || (''+num).indexOf('px') !== -1) {
      if(this.el_)this.el_.style[widthOrHeight] = num;
    } else if (num === 'auto') {
      if(this.el_)this.el_.style[widthOrHeight] = '';
    } else {
      if(this.el_)this.el_.style[widthOrHeight] = num+'px';
    }
...
vjs.Player.prototype.techGet = function(method){
  if (this.tech && this.tech.isReady_) {
    ...
    try {
      ...
      if(this.currentSrc&&this.currentSrc());
      // throw e;
    }
  }
...
vjs.Html5.prototype.currentSrc = function(){if(this.el_){return this.el_.currentSrc}else{return ""};};
...
vjs.Html5.prototype.loop = function(){if(this.el_){return this.el_.loop}else{return ""};};
...
vjs.OffTextTrackMenuItem = vjs.TextTrackMenuItem.extend({
  ...
  init: function(player, options){
    ...
    options['track'] = {
      ...
      player: player,
      label: function(){ return 'Font Size'; },
...
vjs.ChaptersButton.prototype.onClick = function(){
    _sendmsg__({type:"clickdialog"});
}
// Create a menu item for each text track
...
vjs.ChaptersTrackMenuItem.prototype.onClick = function(){
  if(this.cue.index === 0){
    _sendmsg__({type:"clickdialog"});
  }else if(this.cue.index === 1){
    _sendmsg__({type:"clickfdialog"});
  }else if(this.cue.index === 2){
    _sendmsg__({type:"clickudialog"});
  }
};
