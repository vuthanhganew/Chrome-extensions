...
(function() {
    ...
    chrome.sockets.tcp.onReceiveError.addListener( onTCPEroorReceive )
    ...
    IOStream.prototype = {
        ...
        tryWrite: function(callback) {
            ...
            setTimeout(function(){
            sockets.tcp.send( that.sockId, data, that.onWrite.bind(that, callback) )
            },0)
        },
        onWrite: function(callback, evt) {
            ...
            if (this.writeBuffer.size() > 0) {
                if (!this.closed)this.tryWrite(callback)
            } else {
